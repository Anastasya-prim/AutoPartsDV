...
import { searchParts } from "@/lib/mock-data";
import type { SearchResult } from "@/lib/types";

/* ============================================================
   Страница результатов поиска /search?q=...
   Таблица предложений (точные + аналоги) с сортировкой и фильтрами.
   Адаптивная: на мобильных каждый результат — карточка.
   ============================================================ */
...
export default function SearchPage() {
  ...
  const allResults = useMemo(() => searchParts(query), [query]);

  const filtered = useMemo(() => {
    let items = onlyInStock ? allResults.filter((r) => r.inStock) : allResults;
    ...
    <div className="max-w-6xl mx-auto px-4 py-6 sm:py-8">
      ...
      <div className="mb-4">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Найдено {allResults.length} предложений — {exact.length} точных, {analogs.length} аналогов
        </p>
      </div>
