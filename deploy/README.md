...
3. Из контейнера **nginx** до **api** (обходит хост и проброс портов):  
   `docker compose exec nginx wget -qO- http://api:4000/api/health`  
   Ожидается `{"ok":true}`. Если здесь ОК, а `curl http://127.0.0.1:4000` даёт **Connection reset** — смотрите проброс портов / iptables на VPS; для проверки временно в `docker-compose.yml` замените `127.0.0.1:4000:4000` на `4000:4000` и снова `docker compose up -d`.
